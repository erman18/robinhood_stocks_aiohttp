...
class Robinhood(ApiOperations):
    from Robinhood.authentication import login, respond_to_challenge

    from Robinhood.account import (
        get_all_positions,
    ...
    from Robinhood.options import (
        ...
        find_tradable_options2,
    )

    from Robinhood.crypto import (
        load_crypto_profile,
    ...
    from Robinhood.custom import (
        get_option_detail_from_position_filter,
        clear_screen,
        historical_filter,
        get_option_detail_from_current_option_positions,
        historical_filter,
        open_option_positions_filter,
        display_current_status_of_stock_list,
        get_stock_positions_from_account,
        get_list_of_instruments,
        get_current_status_of_stock_list,
        get_option_positions_from_account,
    )
    ...
    async def id_for_stock(self, symbol):
        ...
        data = await self.custom_async_get_wild(
            url, "indexzero", headers=self.default_header, params=payload
        )
        return self.data_filter(data, "id")

    async def custom_async_get_wild(
        self, url, data_type="regular", headers=None, params=None, jsonify_data=True
    ):
        if not jsonify_data:
            res = await self.async_get_wild(
                url, headers=headers, params=params, jsonify_data=False
            )
            return res

        res = await self.async_get_wild(
            url, headers=headers, params=params, jsonify_data=True
        )
        ...
        if data_type == "results":
            ...
            while res.get("next"):
                res = await self.async_get_wild(
                    res["next"], headers=headers, params=params, jsonify_data=True
                )
                count += 1
                if count > 50:
                    break
    ...
    async def id_for_chain(self, symbol):
        ...
        payload = {"symbol": symbol}
        url = "https://api.robinhood.com/instruments/"
        data = await self.async_get_wild(
            url, headers=self.default_header, params=payload, jsonify_data=True
        )
    ...
    async def id_for_group(self, symbol):
        ...
        data = await self.async_get_wild(
            url, headers=self.default_header, jsonify_data=True
        )
    ...
    def filtering_options_body(each_option_owned, option_details, option_description):
        ...
        return {
            ...
            "percent_gain": calc_option_percentage(
                float(each_option_owned["average_price"]),
    ...
    def inputs_to_set(input_symbols):
        ...
        if type(input_symbols) is str:
            ...
            type(input_symbols) is list
            or type(input_symbols) is tuple
            or type(input_symbols) is set
        ):
